...
## php数据类型
- 标量类型：
  - 整型（integer）：用于存储整数。例如，$age = 25;
  - 浮点型（float 或 double）：用于存储浮点数（小数）。例如，$price = 19.99;
  - 字符串（string）：用于存储文本。例如，$name = "John Doe";
  - 布尔型（boolean）：用于存储真或假的值。可以是 true 或 false。例如，$isLoggedIn = true;
- 复合类型：
  - 数组（array）：用于存储一组有序的值，这些值可以是不同类型的。例如，$fruits = array("apple", "banana", "cherry");
  - 对象（object）：用于存储通过类（class）实例化的对象的属性和方法。例如，通过创建一个类并实例化它：
- 特殊类型
  - 空值（NULL）：用于表示一个变量没有值。例如，$variable = NULL;
  - 资源（resource）：用于存储外部资源的一个引用，比如数据库连接、文件句柄等。例如，通过打开文件获得资源：
  - 可调用（callable）：这是一个特殊的类型，用来表示任何可以作为函数调用的东西，比如一个匿名函数、普通函数或类的方法。例如，$callback = function($x) { return $x * 2; };

## php预定义变量
在PHP中，预定义变量（也称为超全局变量）是PHP在脚本执行期间自动创建的特殊变量。这些变量可以在脚本的任何地方访问，而无需事先声明。它们主要用于收集来自用户输入、服务器信息、会话和Cookie等的数据。以下是PHP中的几个主要预定义变量（超全局变量）

- $_GET：
用于收集通过URL参数传递的数据。
例如，如果URL是http://example.com/page.php?id=123&name=John，则$_GET['id']的值为123，$_GET['name']的值为John。
- $_POST：
用于收集通过HTTP POST方法传递的数据，通常用于表单提交。
数据不会显示在URL中，比$_GET更安全。
- $_REQUEST：
包含$_GET、$_POST和$_COOKIE数组的内容。
注意：不建议在生产环境中使用$_REQUEST，因为它可能会引发安全问题，特别是当无法确定数据来自何处时。
- $_COOKIE：
用于收集存储在客户端浏览器中的Cookie数据。
例如，如果设置了一个名为user的Cookie，则可以通过$_COOKIE['user']访问它。
- $_SESSION：
用于存储会话数据，数据在服务器端保存，并通过会话ID与客户端关联。
在使用前需要通过session_start()函数启动会话。
- $_SERVER：
包含服务器和执行环境的信息，如头部信息、路径和脚本位置等。
例如，$_SERVER['SERVER_NAME']包含服务器的名称，$_SERVER['REQUEST_URI']包含请求的URI。
- $_FILES：
用于收集通过HTTP POST方法上传的文件信息。
这是一个多维数组，包含文件的名称、类型、大小、临时存储位置等信息。
- $_GLOBALS：
是一个包含了全局作用域中所有变量的数组。
不建议在编写代码时使用$_GLOBALS，因为它会使代码难以维护和理解，并可能引发安全问题。
- $_ENV：
包含了服务器环境的变量信息，这些变量通常来源于服务器的配置文件（如php.ini或环境变量）。
默认情况下，这个数组可能为空，因为PHP配置可能禁用了对环境变量的访问。
在编写PHP脚本时，正确使用这些预定义变量是非常重要的，因为它们提供了与用户、服务器和会话交互的关键方式。同时，也要注意安全性，特别是在处理用户输入时，要防止SQL注入、跨站脚本（XSS）等安全漏洞。
...
## mysql表连接
表示例：
![alt inner-join结果](/images/test_as.png)
![alt inner-join结果](/images/test_bs.png)

### 内连接
inner join内连接，结果集是两表有关联的数据
![alt inner-join结果](/images/inner_join_result.png)

### 外连接
left join左外连接和right join右外连接，结果集是会显示对应方向表的所有的数据，就算相反方向的表数据有没有关联也还是会显示对应方向表的所有数据，没有关联的字段null填充
![alt left-join结果](/images/left_join_result.png)
